...
This is the experimental Lua 5.1.1 branch of the fork. It exists for hosts that
embed Lua 5.1 or accept a Lua 5.1-compatible runtime, with mpv as the first
non-security/non-networking proof target. Use the versioned branch that matches
the host application you are relinking.
...
Build Nmap against a matching `liblua-mcp` runtime, then start a local MCP
endpoint from NSE. The validated Nmap alpha proof currently lives on the Lua
5.4.8 branch. One local build pattern is:
...
`liblua-mcp` prefix. The validated HAProxy proof currently lives on the Lua
5.4.8 branch.
...
mpv accepts Lua 5.1/5.2-style scripting runtimes, which makes it a useful proof
that the liblua-mcp idea is not limited to security tooling.

Build mpv against this Lua runtime by exposing a local pkg-config file for the
compiled `liblua.a`, then load:

```sh
LUA_MCP_ENABLE=1 LUA_MCP_CONTROL=1 \
LUA_MCP_SOCKET=/run/user/1000/liblua-mcp/mpv.sock \
mpv --idle=yes --script=/path/to/lua-mcp/examples/mpv/mcp-listen.lua
```

The mpv activator registers semantic tools such as:

- `mpv_mcp_info`
- `mpv_property_snapshot`
- `mpv_get_property`
- `mpv_pause`
- `mpv_unpause`
- `mpv_seek_forward_5`
...
- The mpv activator starts a blocking listener from an mpv script callback; this
  is acceptable for the first local proof but should become event-loop friendly.
...
This branch is based on the upstream Lua 5.1.1 source tree. For complete
