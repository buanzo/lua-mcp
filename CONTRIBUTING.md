...
local-only MCP exposure for embedded Lua hosts, with the latest Lua development
branch as the canonical runtime surface.

This repository is not intended to send pull requests to upstream `lua/lua`.
If GitHub shows a fork banner or a "Compare & pull request" button, verify the
base repository before doing anything. Experiment feedback belongs in
`buanzo/lua-mcp`.

## Good first contributions

- Reproduce the build on another Linux distribution.
- Report Lua latest embedding results.
- Report host relinking results on the matching compatibility branch.
...
python3 tools/test_lua_mcp_stdio_bridge.py
git diff --check
```
...
If a change touches a host proof, describe the host version, the liblua-mcp
branch used, and the exact activator command used for validation.
...
Lua with minimal friction, but do not target upstream `lua/lua` with MCP
